...
def problem5(seq_of_seq):
    ...
    """
    ########## My code only flips out when there is a string of more than 2
    newlist = []
    for k in range(len(seq_of_seq)):
        for j in range(len(seq_of_seq[k])):
            if len(seq_of_seq[k]) == 1:
                newlist = newlist + seq_of_seq[0]
            theseq = seq_of_seq[k][j]
            if theseq > seq_of_seq[k][j - 1]:
                newlist = newlist + [seq_of_seq[k][j]]
            else:
                newlist = newlist
    return newlist
    # -------------------------------------------------------------------------
